...
def fetch_playerstat_for_player(season, gameid, playerid):
    ...
    for i, row in df.iterrows():
        if row["Unnamed: 0"] == playerid:  #Unamed : 0 is the column name for player ids
            ...
            minutes_1 = convert_time_to_decimal(game_df.iloc[0]["away_minutes_played"])
            minutes_2 = convert_time_to_decimal(game_df.iloc[0]["home_minutes_played"])
...
def fetch_data_for_game(season, gameid, isplayoff):
    ...
    if isplayoff:
        for index, row in player_df.iterrows():
            ...
            if not math.isnan(row["minutes_played"]):
...
def fetch_data_for_all_games(season, date):
    ...
    po_values = []
    gameids = []
    games_to_compute = pd.DataFrame(columns=["gameid", "date", "isplayoff"])
    for team in teams:
        ...
        for index, row in df.iterrows(): 
            ...
            if game_date < (datetime.today() - timedelta(days=1)):
                games_to_compute =  games_to_compute.append({"gameid":gameid, "date":game_date.date(), "isplayoff":isplayoff}, ignore_index=True)          
            else:
                continue
...
def fetch_injured_player_list():
    ...
    wb = xlapp.Workbooks.Open(f'{path}\\NBAInjuriesFile.xlsx') 
    wb.RefreshAll() 
